...
//     Material()
//         : ks(0), kd(0), specularexponent(0), diffusecolor(Vec3(0, 0, 0)), specularcolor(Vec3(0, 0, 0)),
//           isreflective(false), isrefractive(false), reflectivity(0), refractiveindex(0) {}

//     Material(float ks, float kd, float spec_exp, const Vec3 &diffuse, const Vec3 &specular, bool reflective, bool refractive, float reflect, float refract_index)
//         : ks(ks), kd(kd), specularexponent(spec_exp), diffusecolor(diffuse), specularcolor(specular),
//           isreflective(reflective), isrefractive(refractive), reflectivity(reflect), refractiveindex(refract_index) {}
...
//           refractiveindex(mat_json.value("refractiveindex", 0.0f)) {}
...
// {
// public:
//     Diffuse(const Vec3 &albedo) : Material(0, 1, 0, albedo, Vec3(0, 0, 0), false, false, 0, 0) {}

//     Diffuse(const nlohmann::json &mat_json) : Material(mat_json) {}
...
//     Metal(const Vec3 &albedo, float f, float reflectivity = 1.0)
//         : Material(1, 0, 0, albedo, albedo, true, false, reflectivity, 0), fuzz(f < 1 ? f : 1) {}
...
//         // Use specular color and reflectivity for mirror-like reflection
//         attenuation = specularcolor * reflectivity;
...
//     Metal(const Vec3 &albedo, float f) : Material(1, 0, 0, albedo, albedo, true, false, 1.0f, 0), fuzz(f < 1 ? f : 1) {}

//     Metal(const nlohmann::json &mat_json)
//         : Material(mat_json),
//           fuzz(mat_json.value("fuzz", 0.0f)) {}
...
// {
// public:
//     Dielectric(float refractive_index) : Material(0, 0, 0, Vec3(1.0, 1.0, 1.0), Vec3(1.0, 1.0, 1.0), false, true, 0, refractive_index) {}

//     Dielectric(const nlohmann::json &mat_json) : Material(mat_json) {}
